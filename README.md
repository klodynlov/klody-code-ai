...
| **Ops / runtime** | `docker_control` (introspection Docker **lecture seule** : ps, images, inspect, logs, stats), `kubectl_control` (introspection Kubernetes **lecture seule** : get, describe, logs, top…), `git_control` (Git : status, log, diff, show, blame… en lecture seule ; add/commit locaux si `GIT_WRITE_ENABLED`) |
