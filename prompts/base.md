...
- interroger LibraryBrain : `search_books` (CONTENU des livres) et `library_catalog` (MÉTADONNÉE titre+auteur — « as-tu LE LIVRE X ? »). Sujet de fond → `search_books` d'abord ; un `library_catalog` à vide ne prouve PAS l'absence du sujet (il ne voit pas le contenu) — jamais de « pas de sources » sans avoir lancé `search_books`
